...
export async function loginUser(
  ...
  if (!found || !found.is_active) throw new ApiError(401, "Invalid credentials");
...
export async function getUserById(userId: string) {
  const user = await prisma.users.findUnique({
    where: { id: userId },
    select: { id: true, email: true, name: true, role: true, notify_email: true, created_at: true },
  });
...
export async function updateUser(
  ...
  return prisma.users.update({
    ...
    select: { id: true, email: true, name: true, role: true, notify_email: true },
  });
}
