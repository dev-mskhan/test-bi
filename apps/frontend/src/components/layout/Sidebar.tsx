...
import { BarChart3, Users, LogOut, Sparkles } from "lucide-react";
...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    { to: "/", label: "Analyses", icon: BarChart3, end: true },
  ];
  ...
  return (
    <aside className="w-64 bg-[#1a1a1a] border-r border-[#2a2a2a] flex flex-col h-screen text-[#ededed]">
      <div className="h-16 flex items-center gap-2 px-6 border-b border-[#2a2a2a]">
        ...
        <span className="font-semibold text-lg tracking-tight">AI BI Platform</span>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            ...
            className={() =>
              `flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${isAnalysesActive
      ...
      <div className="p-4 border-t border-[#2a2a2a] bg-[#141414] flex flex-col gap-3">
        <div className="flex items-center gap-3 px-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-[#888888] truncate capitalize">{user?.role}</p>
          </div>
        </div>
        <button
